...
public class Graph {
    ...
    private void createAndConnectEdges() {
                ...
                for (Line line : lines){
                    ...
                    for (int i = 0; i<line.getStops().size()-1; i++){
                        ...
                        Edge e = new Edge(fromStop, toStop, line.getType(), line, times.get(i+1)-time, fromStop.distanceTo(toStop));
                        

                        edges.add(e);
                        fromStop.addForwardEdge(e);
                        toStop.addBackwardEdge(e);
                    }
                }
    }
    ...
    public void computeNeighbours(){
        // TODO
        for(Stop stop: stops){
            if(stop.getBackwardEdges().size()>0){
                for(Edge backEdge :stop.getBackwardEdges()){
                    stop.addNeighbour(backEdge.fromStop());;
                }
            }
            if(stop.getForwardEdges().size()>0){
                for(Edge forwardEdge :stop.getForwardEdges()){
                    stop.addNeighbour(forwardEdge.toStop());
                }
            }
    ...
    public void recomputeWalkingEdges(double walkingDistance) {
        ...
        for(Stop s : stops){
            for(Stop s2 : stops){
                if(s.getId()!=s2.getId()){
                    if(s.distanceTo(s2)<=walkingDistance){
                        Edge e = new Edge(s, s2, "WALKING", null, 0, s.distanceTo(s2));
                        //back the other way
                        Edge e2 = new Edge(s2, s, "WALKING", null, 0, s.distanceTo(s2));
                        edges.add(e);
                        s.addBackwardEdge(e);
                        s2.addForwardEdge(e);
                        edges.add(e2);
                        //s.addBackwardEdge(e2);
                        //s.addForwardEdge(e2);
                        count++;
                    }
